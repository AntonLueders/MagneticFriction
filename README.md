...
**Attention:** The angles utilized and stored by the simulation program to characterize the moments of the slider are defined in a different way than in the experiments [1]. In the simulations, the angles start at zero parallel to the x-axis or movement direction, and the positive direction corresponds to a clockwise rotation. Hence, to compare the data to the experiments, the angles stored in **array** must be properly adjusted to represent the correct angle of the experimental setup. For instance, if the angles in **array** are at position $`7\pi/4`$, the arguments of the moments (as defined in the experiments and [1]) are $` \pi / 4 `$.
