...
MagneticFriction is a many-body simulation software that calculates the magnetic friction resulting from the relative motion of two periodic magnetic layers (one with moments that can rotate around an axis perpendicular to the movement direction). It utilizes the Verlet algorithm of Molecular dynamics simulations in combination with a Stokes law modeling for the single particle dissipation and integrates the rotations of the moments of the slider, which is the layer with the moveable moments. MagneticFriction is a part of the supporting information of 
...
**Attention:** The angles utilized and stored by the simulation program to characterize the moments of the slider always have the opposite sign of the real dynamics. I.e., if the moments rotate counterclockwise, the angles rotate clockwise. Hence, to compare the data to the experiments, the angles stored in **array** must be properly adjusted to represent the correct angle of the experimental setup. For instance, if the angles in **array** are at position $ 7\pi/4 $, the correct argument of the moments is $ \pi / 4 $.
...
- **Stokes-like shaft friction** For the dissipation term $ f $ that enters the equations of motion of the particular moments of the slider, a Stokes law $ f(\omgea) = - \gamma \omega $ is utilized.
