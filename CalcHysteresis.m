...
function [T_phi, T_theta, Bx, By, Bz, Mx_phi, My_phi, Mz_phi, Mx_theta, ...
    ...
    T_phi = T_phi / (length(1:2:n) * m);
    T_theta = T_theta / (length(2:2:n) * m);

    Bx = Bx / (n * m);
    By = By / (n * m);
    Bz = Bz / (n * m);

    Mx_phi = sum(m_x(1:2:n,:),'all') / (length(1:2:n) * m)...
       / magnetic_moment;
    My_phi = sum(m_y(1:2:n,:),'all') / (length(1:2:n) * m)...
       / magnetic_moment;
    Mz_phi = sum(m_z(1:2:n,:),'all') / (length(1:2:n) * m)...
       / magnetic_moment;

    Mx_theta = sum(m_x(2:2:n,:),'all') / (length(2:2:n) * m)...
       / magnetic_moment;
    My_theta = sum(m_y(2:2:n,:),'all') / (length(2:2:n) * m)...
       / magnetic_moment;
